...
from cl_fcl_baseline.algorithms.FCL import MultiFCLClient, MultiFCLServer
from cl_fcl_baseline.experiments.args import parse_multifcl_args
...
def main() -> None:
    ...
    if args.backbone_source == "clip":
        ...
        clip_model.eval()
        with torch.no_grad():
            semantic_features = clip_model.encode_text().cpu()
        visual_projection = clip_model.visual_projection.detach().cpu().clone()
        backbone = clip_model.visual
    task_classes = {task.task_id: task.metadata.get("classes", []) for task in tasks}
    clients = [
        MultiFCLClient.build(
            client_id=f"client_{index}",
            backbone=copy.deepcopy(backbone),
            task_loaders=loaders[index],
            task_classes=task_classes,
            adapter_dim=args.fcl_adapter_dim,
            adapter_dropout=args.multifcl_adapter_dropout,
            num_experts=args.multifcl_num_experts,
            device=device,
            epochs=args.local_epochs,
            lr_adapter=args.multifcl_adapter_lr,
            lr_head=args.multifcl_head_lr,
            weight_decay=args.multifcl_weight_decay,
            semantic_features=semantic_features,
            visual_projection=visual_projection,
        )
        for index in range(args.num_clients)
    ]
    initial_state = clients[0].state_dict()
    for client in clients[1:]:
        client.load_state_dict(initial_state)
    server = MultiFCLServer(
        global_state=initial_state,
        clients=clients,
        client_sample_ratio=args.client_sample_ratio,
        seed=args.seed,
    )
    eval_client = copy.deepcopy(clients[0])

    def _evaluate_unit(_client_id: str | None, task_id: str) -> dict[str, float]:
        eval_client.load_state_dict(server.global_state)
        eval_client.backbone.to(device).eval()
        eval_client.adapters.to(device).eval()
        eval_client.expert_heads.to(device).eval()

        def _predict(inputs: torch.Tensor) -> torch.Tensor:
            logits, _ = eval_client._forward(inputs)
            return logits

        seen_task_count = min(len(tasks), int(server.task_index) + 1)
        seen_class_ids = sorted(
            {
                int(class_id)
                for task in tasks[:seen_task_count]
                for class_id in task.metadata.get("classes", [])
            }
        )
        ...
        metrics["task_aware_accuracy"] = metrics["accuracy"]
        metrics["accuracy"] = metrics.pop("seen_accuracy")
        return metrics
    ...
    evaluator = HistoricalTaskEvaluator(
        ...
        model_source="server_global_trainable_state",
        eval_mode="class_incremental_seen_classes",
    )
    _run_experiment(
        args,
        "multifcl",
