...
from cl_fcl_baseline.algorithms.FCL import FedMGPClient, FedMGPServer
from cl_fcl_baseline.experiments.args import parse_fedmgp_args
from cl_fcl_baseline.experiments.FCL.common import (
    HistoricalTaskEvaluator,
    evaluate_classification,
)
from cl_fcl_baseline.models import PrefixPromptPool, PromptPool
...
def main() -> None:
    ...
    for index in range(args.num_clients):
        clients.append(
            FedMGPClient(
                ...
                global_prompt=copy.deepcopy(global_prompt),
                local_prompt=PrefixPromptPool(
                    min(args.fedmgp_local_prompt_layers, args.fcl_depth),
                    num_classes,
                    args.fedmgp_local_prompt_length,
                    args.fcl_embed_dim,
                ),
                local_head=nn.Linear(args.fcl_embed_dim, num_classes),
                task_loaders=loaders[index],
                device=device,
                lr=args.lr,
                epochs=args.local_epochs,
                pull_constraint=args.fedmgp_pull_constraint,
            )
        )
    server = FedMGPServer(
        backbone=copy.deepcopy(backbone),
        global_prompt=copy.deepcopy(global_prompt),
        public_loaders=_build_public_loaders(args, train_sets),
        fusion_epochs=args.fedmgp_fusion_epochs,
        fusion_lr=args.fedmgp_fusion_lr,
        device=device,
        clients=clients,
    ...
    def _evaluate_unit(client_id: str | None, task_id: str) -> dict[str, float] | None:
        ...
        client.global_prompt.load_state_dict(server.global_prompt.state_dict(), strict=True)
        client.backbone.classifier.load_state_dict(
            server.backbone.classifier.state_dict(), strict=True
        )
        client.backbone.to(device).eval()
        client.global_prompt.to(device).eval()
        client.local_prompt.to(device).eval()
        client.local_head.to(device).eval()
    ...
    evaluator = HistoricalTaskEvaluator(
        ...
        model_source="server_global_prompt_plus_client_local_prompt_head",
    )
    _run_experiment(
        args,
        "fedmgp",
