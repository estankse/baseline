...
from cl_fcl_baseline.algorithms.FCL import FedProTIPClient, FedProTIPServer
from cl_fcl_baseline.experiments.args import parse_fedprotip_args
...
def main() -> None:
    args = parse_fedprotip_args()
    device = _resolve_device(args.device)
    tasks, train_sets, test_sets, input_shape, num_classes = _build_task_stream(args)
    client_loaders = _build_client_loaders(args, tasks, train_sets)
    ...
    for index in range(args.num_clients):
        model = build_model_from_args(args, input_shape, num_classes)
        optimizer = (
        ...
        clients.append(
            FedProTIPClient(
                ...
                model=model,
                optimizer=optimizer,
                task_loaders=client_loaders[index],
                device=device,
                epochs=args.local_epochs,
                threshold=args.fedprotip_threshold,
                activation_batches=args.fedprotip_activation_batches,
                activation_columns=args.fedprotip_activation_columns,
            )
        )
    server = FedProTIPServer(
        model=build_model_from_args(args, input_shape, num_classes),
        clients=clients,
    ...
    evaluator = HistoricalTaskEvaluator(
        ...
        model_source="server_global_model",
    )
    _run_experiment(
        args,
        "fedprotip",
