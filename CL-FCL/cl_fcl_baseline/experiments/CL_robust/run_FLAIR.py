...
from cl_fcl_baseline.algorithms.CL_robust.FLAIR import FLAIRLearner
...
try:
    from ..args import parse_flair_args
except ImportError:  # pragma: no cover
    from cl_fcl_baseline.experiments.args import parse_flair_args
...
def main() -> None:
    ...
    learner = FLAIRLearner(
        ...
        distillation_weight=args.flair_alpha,
        flatness_weight=args.flair_beta,
    )
    ...
    if not log_path:
        ...
        log_path = str(Path("logs") / f"cl_robust_flair_{stamp}.jsonl")
    else:
    ...
    print(f"FLAIR finished. log={log_path}")
