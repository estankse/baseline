...
from cl_fcl_baseline.algorithms.CL_robust.DAML import DAMLLearner
...
try:
    from ..args import parse_daml_args
except ImportError:  # pragma: no cover
    from cl_fcl_baseline.experiments.args import parse_daml_args
...
def _build_task_stream(
    ...
    num_tasks = (
        int(args.num_tasks)
        if args.num_tasks > 0
        else total_num_classes // args.classes_per_task
    )
...
def main() -> None:
    ...
    learner = DAMLLearner(
        ...
        additional_memory_weight=args.daml_alpha,
    )
    ...
    if not log_path:
        ...
        log_path = str(Path("logs") / f"cl_robust_daml_{stamp}.jsonl")
    else:
    ...
    for task_index, task in enumerate(tasks):
        ...
        def _evaluate(epoch: int) -> tuple[dict[str, float], dict[str, float]]:
        ...
        def _on_epoch(metrics: dict[str, float]) -> None:
            ...
            if args.eval_every > 0 and completed_epochs % int(args.eval_every) == 0:
                # DAML inherits iCaRL's herding classifier. Build temporary
                # current-task class means for epoch diagnostics, then restore
                # the old-task replay memory so evaluation cannot alter the
                # samples used by subsequent optimization steps.
                saved_sets = {
    ...
    print(f"DAML finished. log={log_path}")
