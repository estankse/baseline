...
from cl_fcl_baseline.algorithms.CL.ewc import EWCLearner
...
try:
    from ..args import parse_ewc_args
except ImportError:  # pragma: no cover
    from cl_fcl_baseline.experiments.args import parse_ewc_args
...
def _build_task_stream(
    ...
    if args.scenario == "domain":
        # The EWC paper's supervised benchmark keeps the output domain fixed
        # and creates tasks with deterministic random pixel permutations.
        # The same permutation must be shared by a task's train/test split.
        num_tasks = int(args.num_tasks) if args.num_tasks > 0 else 10
        flattened_size = 1
        for dimension in input_shape:
            flattened_size *= int(dimension)
        tasks: list[TaskDefinition] = []
        train_datasets: dict[str, Dataset] = {}
        test_datasets: dict[str, Dataset] = {}
        shared_classes = list(range(total_num_classes))
        for task_index in range(num_tasks):
            task_id = f"task_{task_index}"
            generator = torch.Generator().manual_seed(int(args.seed) + task_index)
            permutation = torch.randperm(flattened_size, generator=generator)
            tasks.append(
                TaskDefinition(
                    task_id=task_id,
                    name=task_id,
                    num_classes=total_num_classes,
                    metadata={"classes": shared_classes},
                )
            )
            train_datasets[task_id] = PermutedPixelsDataset(train_dataset, permutation)
            test_datasets[task_id] = PermutedPixelsDataset(test_dataset, permutation)
...
def _continual_metrics(
    ...
    for task_index, task_id in enumerate(list(current)[:-1]):
        previous_best = max(row[task_id] for row in accuracy_matrix if task_id in row)
        forgetting.append(previous_best - current[task_id])
...
def main() -> None:
    ...
    if args.scenario == "class":
        warnings.warn(
            "EWC alone does not correct the new-class logit bias in replay-free "
            "class-incremental learning. Use the paper-compatible defaults, or "
            "--scenario task for a split-class benchmark with task identity.",
            stacklevel=2,
        )
    set_seed(args.seed)
    device = torch.device(
    ...
    if not log_path:
        ...
        log_path = str(Path("logs") / f"cl_ewc_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jsonl")
    else:
    ...
    print(f"EWC finished. log={log_path}")
