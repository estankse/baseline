...
from cl_fcl_baseline.algorithms.CL.iCaRL import ICaRLLearner
...
try:
    from ..args import parse_icarl_args
except ImportError:  # pragma: no cover
    from cl_fcl_baseline.experiments.args import parse_icarl_args
...
def _continual_metrics(
    ...
    current_average = sum(current.values()) / max(1, len(current))
    stage_averages = [
        sum(row.values()) / max(1, len(row)) for row in accuracy_matrix
    ]
    average_incremental_accuracy = (
        sum(stage_averages) + current_average
    ) / float(len(stage_averages) + 1)
    ...
    return {
        "average_accuracy": current_average,
        # This is the paper's headline metric: mean cumulative accuracy over
        # every completed class-incremental stage, not just the final stage.
...
def main() -> None:
    ...
    learner = ICaRLLearner(
        ...
        prototype_flip=args.prototype_flip,
    )
    log_path = args.log_file.strip()
    if not log_path:
        Path("logs").mkdir(parents=True, exist_ok=True)
        log_path = str(Path("logs") / f"cl_icarl_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jsonl")
    else:
    ...
    for task_index, task in enumerate(tasks):
        ...
        def _evaluate(epoch: int) -> dict[str, float]:
            ...
            print(
                ...
                f"aia={summary['average_incremental_accuracy']:.4f} "
        ...
        def _on_epoch(metrics: dict[str, float]) -> None:
            ...
            if args.eval_every > 0 and completed_epochs % int(args.eval_every) == 0:
                # Intermediate evaluation is diagnostic only.  Exemplar
                # management belongs after representation training (paper
                # Algorithm 2), so restore both learner and RNG state after
                # constructing temporary NME prototypes.
                saved_sets = {
                    class_id: list(exemplars)
                    for class_id, exemplars in learner.exemplar_sets.items()
                }
                saved_prototype_classes = list(learner._prototype_classes)
                saved_prototypes = learner._prototypes
                saved_current = learner._exemplars_current
                cpu_rng_state = torch.get_rng_state()
                cuda_rng_states = (
                    torch.cuda.get_rng_state_all() if torch.cuda.is_available() else None
                )
                try:
                    learner.update_exemplar_sets(train_loaders[task.task_id])
                    last_eval_row = _evaluate(completed_epochs - 1)
                    last_eval_epoch = completed_epochs
                finally:
                    learner.exemplar_sets = saved_sets
                    learner._prototype_classes = saved_prototype_classes
                    learner._prototypes = saved_prototypes
                    learner._exemplars_current = saved_current
    ...
    print(f"iCaRL finished. log={log_path}")
