...
from cl_fcl_baseline.algorithms.CL.gem import GEMLearner
...
try:
    from ..args import parse_gem_args
except ImportError:  # pragma: no cover
    from cl_fcl_baseline.experiments.args import parse_gem_args
...
def main() -> None:
    ...
    if not log_path:
        ...
        log_path = str(Path("logs") / f"cl_gem_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jsonl")
    else:
    ...
    print(f"GEM finished. log={log_path}")
